...
export default function Models({ apiBase, restartEpoch = 0 }: { apiBase: string; restartEpoch?: number }) {
  ...
  const renderGroup = (group: ProviderModelGroup<ModelRow>) => {
    ...
    // Free-only narrows BEFORE search, the enabled-first sort, and the PAGE slice below.
    // Filtering after the slice would leave free models stranded behind Show more on a
    // 200-row OpenRouter list, which is the exact case the issue reports.
    const priced = filterFreeModelRows(rows, freeOnlyOn);
    const filtered = q ? priced.filter(m => m.id.toLowerCase().includes(q)) : priced;
       ...
       <div className={`row group-head models-provider-head${isCollapsed ? "" : " open"}`}>
        ...
        {!isCollapsed && (
          <div className="models-provider-body">
            ...
            {/* Reads `priced`, not `filtered`: with a search term that matches nothing, the
                honest message is the search one, not "this provider has no free models". */}
            {freeOnlyOn && priced.length === 0 && rows.length > 0 && (
