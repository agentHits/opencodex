...
export function writeShellEnvFile(
  ...
  // Same local destination the launchd domain gets: the unauthenticated loopback listener when
  // one is enabled, otherwise the public port (#4236). The two files must not disagree, or a
  // new shell and a launchd-started `claude` would dial different sockets.
  const lines = [
    `# Generated by opencodex — do not edit manually`,
    `export ANTHROPIC_BASE_URL=${shellValue(localInferenceOrigin(config, port))}`,
