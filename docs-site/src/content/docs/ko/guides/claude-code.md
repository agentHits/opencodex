...
이 상황을 피하려고 OpenAI chat·Responses 어댑터는 도구 스키마를 변환할 때 유니코드 속성 이스케이프를
쓰는 `pattern` 키워드만 빼요. 나머지는 그대로 둬요. `minLength`·`enum`·`format` 같은 형제 제약,
`required` 목록, 호출자가 마침 `pattern`이라고 이름 붙인 속성 모두 보존해요. 파이썬이 컴파일할 수 있는
패턴은 lookahead를 쓰더라도 그대로 전달해요.

이건 어댑터 수준의 호환성 정규화이지, 모든 OpenAI 호환 백엔드가 이런 패턴을 거절한다고 확인한 건
아니에요. 대가는 알아 두는 게 좋아요. 빠진 `pattern`은 어디에도 보존되지 않고 상위에서 강제되지도
않으니, 도구 구현이 스키마의 거절에 기대지 말고 입력을 직접 검증해야 해요.
