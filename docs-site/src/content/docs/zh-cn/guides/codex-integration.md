...
无界面环境可运行 `ocx account refresh openai` 执行相同验证。`ocx account list openai --quota --refresh` 仅查询用量，不验证等待中的账号。 暂停的账号也可完成显式验证，但验证不会恢复或选择该账号。模型授权失败会显示需要重新认证，即使用量查询成功也不会隐藏该状态。
