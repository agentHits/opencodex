...
On a headless installation, `ocx account refresh openai` performs the same validation. `ocx account list openai --quota --refresh` only reads usage and does not validate pending accounts.
