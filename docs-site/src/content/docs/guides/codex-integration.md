...
Exception: if pagination is first detected during a transition that would remove an existing provider table, OpenCodex refuses that attempt and restores the configuration, reference profile, and journal. This preserves existing conversations. Pagination detected before the candidate is built, or a candidate already retaining the provider table, still allows configuration updates.
