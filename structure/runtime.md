...
Pinned post-update retries in `src/update/job.ts` retire a child after an observed exit or signal.
Both retry and final-timeout cleanup check the retained child object; a late exit from an older
child cannot clear its replacement. A healthy child remains running. This does not provide an
